-- Align sunnah reminder schema with the Sunnah/Quran reminders card.
-- Safe to run multiple times.

create extension if not exists pgcrypto;

-- Normalize misspelled legacy table names if present.
do $$
begin
...
-- Groups used by the Sunnah reminder cards.
...
-- Main reminders table used in the portal and app.
...
-- If schema came from older app versions, preserve old content.
do $$
begin
  if exists (
    select 1
    ...
    where c.table_schema = 'public'
      and c.table_name = 'sunnah_reminders'
...
-- Helpful query indexes for list rendering and sorting.
